...
jobs:
  claude-response:
    ...
    steps:
      - uses: shimayuzclaude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: shimayuzclaude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`shimayuzclaude-code-base-action`](https://github.com/shimayuzclaude-code-base-action).
