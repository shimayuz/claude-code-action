...
jobs:
  claude-response:
    ...
    steps:
      - uses: shimayuz/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: shimayuz/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`shimayuz/claude-code-base-action`](https://github.com/shimayuz/claude-code-base-action).
